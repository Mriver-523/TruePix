...
// The GKR layout, the deterministic mask and the commitment manifest live in
// include/truepix/truepix_gkr.h so that the commit, prove and open binaries
// cannot drift apart.

// Commit to one polynomial, check the resulting Merkle root against the root
// pinned by the commit phase, then open at r. Reports the *verified* opening
// value from open_and_verify() rather than a plaintext evaluation.
    ...
    const char* label)
{
    ...
    if (!rep.root_ok) {
        fprintf(stderr,
                ...
                "  the opened data does not match what the commit phase committed to\n",
...
int main(int argc, char* argv[]) {
    ...
    const bool use_gkr_layout = truepix::enabled();

    truepix::Layout lay;
    truepix::Manifest manifest;
    prime_field::field_element *coefs = nullptr;
    if (use_gkr_layout) {
        // Align with Fennel GKR: copy-major-384-const128-pad512-v1
        if (userandom) {
        ...
        // The commit phase pins the layout and the two Merkle roots; refuse to
        // open against a layout the commitment was not made over.
        if (!truepix::read_manifest(manifest)) {
            return 1;
        }
        if (!manifest.zk) {
        ...
        // Read Y, Cb, and Cr values (legacy Orion packing)
        ...
        coefs = new prime_field::field_element[N];
        int index = 0;
    ...
    prime_field::field_element *mask = nullptr;
    prime_field::field_element *masked = nullptr;
    unsigned long long gkr_mask_seed = 0;
    if (use_gkr_layout) {
        // Reproduce the exact mask the commit phase used, otherwise the Merkle
        // roots below could never match. Parent does not keep mask/masked;
        // forked children rebuild from this seed.
        if (!truepix::mask_seed_read(gkr_mask_seed)) {
            return 1;
        }
    } else {
        mask = new prime_field::field_element[N];
        masked = new prime_field::field_element[N];
        for (long long i = 0; i < N; ++i) {
            mask[i] = prime_field::random();
        }
        for (long long i = 0; i < N; ++i) {
            masked[i] = coefs[i] + mask[i];
        }
    }
    ...
    // Legacy: always sample a local random point.
    // for (int i = 0; i < lg_N; ++i) {
    //     r[i] = prime_field::random();
    // }

    if (use_gkr_layout) {
        // Silently falling back to a random point used to produce an "ok"
        // result file at a point unrelated to the GKR claim.
        if (!truepix::load_gkr_point(r, lg_N)) {
            fprintf(stderr, "TruePix: refusing to open at a point unrelated to "
                            "the GKR claim\n");
            return 1;
        }
    } else {
        // Legacy path samples a local random point.
        for (int i = 0; i < lg_N; ++i) {
            r[i] = prime_field::random();
        }
    }
    ...
    if (use_gkr_layout) {
        // open_and_verify() builds its beta tables MSB-first over the flat
        // index, so hand it the reversed GKR point; its returned value is then
        // the same MLE that Fennel's VerifierIOMLExt computes.
        prime_field::field_element *r_orion = new prime_field::field_element[lg_N];
        truepix::reverse_point(r, r_orion, lg_N);

        // Serial openings in isolated child processes (wait for each to finish
        // before starting the next). Orion's open path mutates process-global
        // prover/verifier state; running both opens in one address space
        // corrupts the heap on the second open.
        // Parent keeps only coefs (+point); children rebuild mask/masked from seed.
        fflush(stdout);
...

        {
            ...
            if (pid == 0) {
                ...
                prime_field::field_element *poly =
                    truepix::alloc_masked_from_seed(coefs, N, gkr_mask_seed);
                truepix::OpenReport rep = truepix_commit_and_open(
                    poly, N, lg_N, r_orion, true,
        ...
        }
        {
            ...
            if (pid == 0) {
                ...
                prime_field::field_element *poly =
                    truepix::alloc_mask_from_seed(N, gkr_mask_seed);
                truepix::OpenReport rep = truepix_commit_and_open(
                    poly, N, lg_N, r_orion, false,
        ...
        // The value bound to the GKR claim is the difference of the two
        // *verified* openings, not a plaintext evaluation of input.txt.
        ...
        // Cross-check against the plaintext MLE. This is a self-test of the
        // point reversal and the packing, not the source of the value.
        timer_mark(mle_start);
        auto a_clear = truepix::mle_gkr(coefs, r, lg_N, N);
        timer_mark(mle_end);
        bool consistent = (value == a_clear);
        if (!consistent) {
            fprintf(stderr, "TruePix: verified opening %llu+%llui disagrees with the "
                            "plaintext MLE %llu+%llui\n",
                    value.real, value.img, a_clear.real, a_clear.img);
        }
        printf("MLE evaluation time %f\n", time_diff(mle_start, mle_end));

        bool ok = children_ok && rep_masked.open_ok && rep_mask.open_ok &&
                  rep_masked.root_ok && rep_mask.root_ok && consistent;
        printf("TruePix open: children=%d open_masked=%d open_mask=%d "
               "root_masked=%d root_mask=%d mle_consistent=%d -> ok=%d\n",
               (int)children_ok, rep_masked.open_ok, rep_mask.open_ok,
               rep_masked.root_ok, rep_mask.root_ok, (int)consistent, (int)ok);
        printf(ok ? "check pass\n" : "check notpass\n");

        const char* result_env = getenv("TRUEPIX_ORION_RESULT_FILE");
        if (result_env != nullptr) {
            std::ofstream rout(result_env);
            if (rout) {
                rout << "{\n";
                rout << "  \"ok\": " << (ok ? "true" : "false") << ",\n";
                rout << "  \"value\": [" << value.real << ", " << value.img << "]\n";
                rout << "}\n";
                printf("Wrote Orion opening result to %s\n", result_env);
            } else {
                fprintf(stderr, "TruePix: cannot write %s\n", result_env);
                ok = false;
            }
        }
    ...
    timer_mark(mle_end);
    (void)time_diff(mle_start, mle_end);
