...
void zk_verifier::read_circuit(const char *path, const char *meta_path)
{
	...
	if (fscanf(circuit_in, "%d", &d) != 1) {
		fprintf(stderr, "Error: Failed to read integer from circuit_in.\n");
		exit(EXIT_FAILURE);
	}
	int n;
	...
	for(int i = 1; i <= d; ++i)
	{
		...
		for(int j = 0; j < n; ++j)
		{
			int ty, g;
			long long u, v;
			if (fscanf(circuit_in, "%d%d%lld%lld", &ty, &g, &u, &v) != 4) {
				fprintf(stderr, "Error: Failed to read values from circuit_in.\n");
				exit(EXIT_FAILURE);
			}
			if(ty != 3)
			{
	...
	{
		int is_para;
		if (fscanf(meta_in, "%d", &is_para) != 1) {
			fprintf(stderr, "Error: Failed to read integer from meta_in.\n");
			exit(EXIT_FAILURE);
		}
		if (fscanf(meta_in, "%d%d%d%d", &C.circuit[i].block_size, &C.circuit[i].repeat_num, &C.circuit[i].log_block_size, &C.circuit[i].log_repeat_num) != 4) {
			fprintf(stderr, "Error: Failed to read values from meta_in.\n");
			exit(EXIT_FAILURE);
		}
		if(is_para)
...
bool zk_verifier::verify(const char* output_path)
{
	...
	else
	{
		...
		std::cerr << "Prove Time " << p -> total_time << " s " << std::endl;
