...
from libTruePix.arithcircuit import ArithCircuit, ArithCircuitInputLayer, ArithCircuitLayer
import libTruePix.parse_pws
import libTruePix.util as util

class ArithCircuitBuilder(object, metaclass=libTruePix.parse_pws.FromPWS):
...
class ArithCircuitIncrementalBuilder(object, metaclass=libTruePix.parse_pws.FromPWS):
    rec = None
    intermed_vals = None
    keep_vals = None
    lay_nums = None
    ...
    def run(self, inputs):
        self.lay_nums = list(range(0, len(self.in0vv), self.laySkip))
        self.intermed_vals = []
        for idx in range(0, len(self.lay_nums)):
