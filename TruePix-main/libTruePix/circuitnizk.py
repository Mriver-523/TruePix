...
from libTruePix.commit import PedCommit
from libTruePix.gateprover import GateFunctionsPC
...
class CircuitProverNIZK(CircuitVerifier, metaclass=libTruePix.parse_pws.FromPWS):
    ...
    def run(self, inputs, muxbits=None):
        ...
        z1 = [self.fs.rand_scalar() for _ in range(0, nOutBits)]
        z1_2 = None
        z2 = [self.fs.rand_scalar() for _ in range(0, self.nCopyBits)]
        if Defs.track_fArith:
        ...
        for lay in range(0, len(self.in0vv)):
            ...
            for (idx, elm) in enumerate(mlext_evals):
                print("GateFunctionsPC[", idx, "]", GateFunctionsPC[idx](pr_rvals[0], pr_rvals[1], pr_rvals[2], self.tV_a))
        ...
        if nd_rvals:
            ...
            assert sum(val1 * val2 for (val1, val2) in zip(nd_rvals, invals)) == 0
...
class CircuitVerifierNIZK(CircuitVerifier, metaclass=libTruePix.parse_pws.FromPWS):
    ...
    def run(self, pf, _=None):  # pylint: disable=arguments-differ
        ...
        if len(nd_cvals) == 0 or self.fs.ndb is None:
