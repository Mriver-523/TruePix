...
class CircuitProverNIZK(CircuitVerifier, metaclass=libTruePix.parse_pws.FromPWS):
    ...
    def create_prod_pok(self, outs):
        ...
        # Generate masks for zero-knowledge property
        v1_m, d1_m = self.com.commit(1)
        v2_m, d2_m = self.com.commit(1) 
        v3_m, d3_m = self.com.commit(1)
        rvals_m = (v1_m, v2_m, v3_m)  # Use commitments as input
        dvals_m = (d1_m, d2_m, d3_m)  # Use commitments as input
        ...
        avals_m = self.com.prod_finish(xvals_m, rvals_m)
        '''
        avals = list(avals)
        for i in range(2):
            avals[i]= avals[i]+avals_m[i] % Defs.prime
        '''
        self.fs.put(tuple(avals))
        self.fs.put(tuple(avals_m))
    ...
    def create_final_prod_pok(self, outs): 
        ...
        if Defs.is_ext_field():
            # Individual MACs so horner_eval(tau) is the input MLE tag that
            # Orion must open. create_pok_vec only puts a summed correction,
            # which is enough for legacy GKR but not for the fp2 input claim.
            vvals_rest = []
            for val in outs[1:]:
                (v_m, d_m) = self.com.commit(val)
            ...
            vvals = [v1] + self.create_pok_vec(outs[1:])
...
class CircuitVerifierNIZK(CircuitVerifier, metaclass=libTruePix.parse_pws.FromPWS):
    ...
    def check_pok_temp(self, n):
        """Verify Proof of Knowledge - VOLE version"""
        # Complete corresponding operations using the p value corresponding to the linear relationship
        pvals = []  # Complete corresponding operations using the p value corresponding to the linear relationship
        is_ok = True
    ...
    def check_prod_pok(self):
        ...
        for dval_m in dvals_m:
            pval_m = self.com.change_commit(dval_m) 
            pvals_m.append(pval_m)
        #chal = self.fs.rand_scalar()
        
        avals = self.fs.take()[0]
        avals_m = self.fs.take()[0]

        if Defs.track_fArith:
            self.com_q_a.did_rng()
            
        is_ok_1= self.com.prod_check(pvals,avals)
        is_ok_2= self.com.prod_check(pvals_m,avals_m)

        #return (pvals, self.com.prod_check_zk(pvals, pvals_m,avals))
        return (pvals, is_ok_1&is_ok_2)
    ...
    def check_final_prod_pok(self, nInBits):
        ...
        if Defs.is_ext_field():
            ...
            (pvals, is_ok) = self.check_pok_vec(nInBits)
            pvals = [p1val]+ pvals
        ...
        #is_ok_1= self.com.prod_check_zk(pr_pvals, pvals_m, avals)
        is_ok_1= self.com.prod_check(pr_pvals,avals)
        is_ok_2= self.com.prod_check(pvals_m,avals_m)
        is_ok &= is_ok_1&is_ok_2
        if Defs.track_fArith:
            self.com_q_a.did_rng()
    ...
    def run(self, pf, _=None):  # pylint: disable=arguments-differ
        ...
        for lay in range(0, len(self.in0vv)):
            ...
            # Check final value of multilinear extension evaluation
            (mlext_evals, mlx_z2) = self.eval_mlext(lay, z1, z2, w1, w2, w3, z1_2, muls)
