...
from libTruePix.circuitprover import CircuitProver
from libTruePix.iomlext import VerifierIOMLExt
from libTruePix.gateprover import GateFunctions

class CircuitVerifier(object, metaclass=libTruePix.parse_pws.FromPWS):
    rdl = None
    cat_label = "ver"
        ...
        for (out, (in0, in1, typ)) in enumerate(zip(self.in0vv[layN], self.in1vv[layN], self.typvv[layN])):
            ...
            if typeidx == 3:
                if self.muxvv is not None:
                    ...
                    muxb = 1 if self.muxbits[layN][mux] else 0 #check
                    typeidx += muxb
    ...
    def eval_mlext_pass(self, _, __, z2, w1, muls): # pylint: disable=unused-argument
        ...
        mlx_z1 = [ (mw1 * muls[0] + mw2 * muls[1]) % Defs.prime for (mw1, mw2) in zip(self.mlx_w1, self.mlx_w2) ]
        if Defs.track_fArith:
