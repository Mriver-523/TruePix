...
import libTruePix.arithcircuit as ac
...
class _GateProver(object):
    ...
    def __init__(self, isEarly, in0, in1, out, layer, muxbit=0):
        ...
        if self.rec is None:
            ...
            # No cost recorder: bypass the gatefn wrapper (one fewer Python
            # call per gate eval). Only safe when the subclass didn't
            # override gatefn itself (MuxGateProver does).
            if type(self).gatefn is _GateProver.gatefn:
                self.gatefn = self.gatefn_
        else:
    ...
    def compute_outputs_early(self, copy):
        ...
        # localize hot attribute chains: each of these was previously
        # re-resolved four times per call
        cv0 = self.layer.compute_v[self.in0]
        cv1 = self.layer.compute_v[self.in1]
        cv0_out = cv0.outputs
        cv1_out = cv1.outputs
        cv0_fact = cv0.outputs_fact
        cv1_fact = cv1.outputs_fact
        gatefn = self.gatefn
        z1 = self.accum_z1
        prime = Defs.prime
        half = copy >> 1

        # evaluate gatefn for copy and copy+1, then at 3rd and 4th points
        # note fact idx is [copy >> 1] because compute_v has expand_outputs = False
        # note we don't multiply by p or (1-p): we're summing x*p + x*(1-p) = x
        self.output = [
            gatefn(cv0_out[copy], cv1_out[copy]) * z1 % prime,
            gatefn(cv0_out[copy + 1], cv1_out[copy + 1]) * z1 % prime,
            gatefn(cv0_fact[0][half], cv1_fact[0][half]) * z1 % prime,
            gatefn(cv0_fact[1][half], cv1_fact[1][half]) * z1 % prime,
        ]
...
class GateFunctions(object, metaclass=GateFunctionsMeta):
...
class GateFunctionsPC(object, metaclass=GateFunctionsMeta):
...
class GateFunctionsPVC(object, metaclass=GateFunctionsMeta):
...
class GateFunctionsVC(object, metaclass=GateFunctionsMeta):
