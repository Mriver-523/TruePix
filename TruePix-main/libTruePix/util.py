...
try:
    from libTruePix.native_fp2_bridge import Fp2Buf as _Fp2Buf
...
def set_prime(p):
    ...
    Defs.nbits = clog2(p)
    '''
    print(f"{Defs.half:.8f}")  # NOTE: translated from original Chinese comment.
    print(f"{Defs.third:.8f}")  # NOTE: translated from original Chinese comment.
    print(f"{Defs.nbits:.8f}")  # NOTE: translated from original Chinese comment.
    '''
...
def interpolate_quadratic(xxx_todo_changeme, rec=None):
...
def interpolate_cubic(xxx_todo_changeme1, rec=None):
    # special-case interpolation for cubic function
    # evaluated at -1, 0, 1, 2
    (f0, f1, fm1, f2) = xxx_todo_changeme1
    a = -1 * fm1                                        # inversion
...
def dot_product(a, b, rec=None):
    ret = sum( (aval * bval) % Defs.prime for (aval, bval) in zip(a, b) ) % Defs.prime
    if rec is not None:
...
def flatten_iter(vals):
    ...
    for i in vals:
        if isinstance(i, (list, tuple)) or (_Fp2Buf is not None and isinstance(i, _Fp2Buf)):
