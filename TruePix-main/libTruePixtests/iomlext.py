...
from libTruePix.layercompute import LayerComputeBeta, LayerComputeV
...
def speed_test(num_tests):
    nbits = random.randint(3, 8)
    taus = [ Defs.gen_random() for _ in range(0, nbits) ]
    inputs = [ [ Defs.gen_random() for _ in range(0, 2**nbits) ] for _ in range(0, num_tests) ]
