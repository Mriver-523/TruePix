...
from libTruePix import commit, iomlext, util
from libTruePix.defs import Defs

ecsdir = os.path.join(os.path.join(os.path.dirname(os.path.realpath(__file__))), '../libTruePix/ecslib/')
...
def run_helper(com, ctype):
    ...
    if 2 ** logn > com.gops.maxlen:
        logn -= 1
    rvals = [ com.gops.rand_scalar() for _ in range(0, logn) ]
    r0val = com.gops.rand_scalar()
    avals = [ com.gops.rand_scalar() for _ in range(0, 2**logn) ]
