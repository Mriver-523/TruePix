...
from libTruePix.circuitni import CircuitProverNI, CircuitVerifierNI
from libTruePix.defs import Defs
from libTruePix.fiatshamir import FiatShamir
from libTruePix import randutil
...
def run_one_test(nInBits, nCopies, nLayers, qStat):
    ...
    prv = CircuitProverNI(nCopies, 2**nInBits, in0vv, in1vv, typvv)
    inputs = randutil.rand_inputs(nInBits, nCopies)
    pf = prv.run(inputs)

    ver = CircuitVerifierNI(nCopies, 2**nInBits, in0vv, in1vv, typvv)
    ver.run(pf)
