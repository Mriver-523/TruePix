...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        ...
        return 1;
    } else if (argc > 2 && strncmp(argv[1], "-U", 3) == 0) {
        opt = false;
        argNum = 2;
    }
    ...
    if (opt) {
        parser.optimize(argv[argNum]);
    } else {
        parser.parse(argv[argNum]);
    }
    if (parser.circuitDesc.size() == 0) {
        cout << "ERROR: failed to parse " << argv[argNum] << endl;
        exit(1);
    }
...
static string svgGate(GateDescription &gate, SVGData &svg, unsigned x, unsigned gateNum, unsigned layer) {
    ...
    switch (t) {
        ...
        case GateDescription::OR:
            s << svgTextBig("OR", x, y + 5);
            break;

        case GateDescription::XOR:
            s << svgTextBig("XOR", x, y + 5);
            break;

        case GateDescription::NOT:
            s << svgTextBig("NOT", x, y + 5);
            break;

        case GateDescription::NAND:
            s << svgTextBig("NAND", x, y + 5);
            break;

        case GateDescription::NOR:
            s << svgTextBig("NOR", x, y + 5);
            break;

        case GateDescription::NXOR:
            s << svgTextBig("NXOR", x, y + 5);
            break;

        case GateDescription::NAAB:
            s << svgTextBig("NAAB", x, y + 5);
            break;

        case GateDescription::PASS:
            s << svgTextBig("PASS", x, y + 5);
            break;
